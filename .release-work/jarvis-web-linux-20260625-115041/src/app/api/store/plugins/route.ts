...
// - ensureStorePluginTable -> line 31
// - getPluginConfig -> line 36
// - GET -> line 47
// - parseJarvMetadata -> line 189
// [/CodeGuard Feature Index]
...
import { ensureStorePluginTable as ensureStorePluginTableWithLocale, getStorePluginLocalization, pickStoreLocaleText, resolveStoreLocale } from '@/lib/storePluginLocale';
...
async function ensureStorePluginTable() {
  await ensureStorePluginTableWithLocale(prisma);
}
...
export async function GET(request: Request) {
  try {
    ...
    const cookieLocaleMatch = cookieHeader.match(/(?:^|;\s*)(?:NEXT_LOCALE|locale)=([^;]+)/i);
    const requestedLocale = resolveStoreLocale(
      requestUrl.searchParams.get('locale') || cookieLocaleMatch?.[1] || request.headers.get('accept-language')
    );
    // 确定插件目录路径
    // 1. 优先使用环境变量
    ...
    const dbPlugins = await prisma.$queryRaw<StorePluginRecord[]>`
      SELECT "id", "name", "nameZh", "nameEn", "version", "price", "description", "descriptionZh", "descriptionEn", "author", "group", "icon", "featured", "updatedAt"
      FROM "StorePlugin"
      WHERE "isActive" = true
    ...
    for (const file of jarvFiles) {
      try {
        ...
        const localized = getStorePluginLocalization(
          pluginId,
          dbPlugin?.name || metadata.name || pluginId,
          dbPlugin?.description || metadata.description || '暂无描述',
          dbPlugin?.group || metadata.group || config.category || '工具'
        );

        plugins.push({
          id: pluginId,
          name: pickStoreLocaleText({ zh: localized.nameZh, en: localized.nameEn }, requestedLocale, localized.nameZh),
          ...
          description: pickStoreLocaleText({ zh: localized.descriptionZh, en: localized.descriptionEn }, requestedLocale, localized.descriptionZh),
          descriptionZh: localized.descriptionZh,
          descriptionEn: localized.descriptionEn,
          author: dbPlugin?.author || metadata.author || 'Jarvis',
          group: pickStoreLocaleText({ zh: localized.groupZh, en: localized.groupEn }, requestedLocale, localized.groupZh),
    ...
    for (const dbPlugin of dbPlugins) {
      ...
      const localized = getStorePluginLocalization(
        dbPlugin.id,
        dbPlugin.nameZh || dbPlugin.name,
        dbPlugin.descriptionZh || dbPlugin.description,
        dbPlugin.group || '工具'
      );
      plugins.push({
        id: dbPlugin.id,
        ...
        downloads: 0,
        rating: 5,
        features: [localized.descriptionZh],
        fileSize: '已上传',
        lastUpdated: new Date(dbPlugin.updatedAt).toISOString().slice(0, 10),
        compatibility: ['Word 2016+', 'Word 365'],
        tags: [localized.groupZh, '效率', 'Word']
      });
    }
