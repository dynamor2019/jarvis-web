...
// - ProviderBrandIcon -> line 44
// - logoY -> line 124
// - handleUserInfo -> line 379
// - PluginList -> line 676
// - isZh -> line 828
// - expiresAtDate -> line 1235
// - loadAvailableModels -> line 1560
// - FAQItem -> line 2122
// [/CodeGuard Feature Index]
...
async function addQrCenterLogo(qrDataUrl: string): Promise<string> {
  ...
  try {
    const [qrImage, logoImage] = await Promise.all([
      loadImage(qrDataUrl),
      loadImage('/assets/qr-center-logo.jpg'),
    ]);
    const size = 256;
    const logoSize = 54;
    ...
    ctx.beginPath();
    ctx.roundRect(logoX - 6, logoY - 6, logoSize + 12, logoSize + 12, 14);
    ctx.fill();
    ctx.save();
    ctx.beginPath();
...
export function Store({ enableMock = false, initialTab = 'subscription' }: { enableMock?: boolean; initialTab?: 'subscription' | 'talent' }) {
  ...
  useEffect(() => {
    ...
    if (params.get('mock') === 'true') setChannel('mock');
    if ((params.get('tab') || '').toLowerCase() === 'talent') setActiveTab('talent');
  }, []);
  ...
  useEffect(() => {
    ...
    const off = onMessage((msg) => {
      ...
      if (msg.type === 'SKILL_INSTALL_PROGRESS') {
        ...
        setSkillInstallState((prev) => ({
          status: normalizedStatus as 'installing' | 'done' | 'failed',
          progress: Math.max(prev.progress, Math.max(0, Math.min(100, progress))),
          message
        }));
      }
  ...
  useEffect(() => {
    if (skillInstallState.status !== 'installing') return;

    const timer = window.setInterval(() => {
      setSkillInstallState((prev) => {
        if (prev.status !== 'installing' || prev.progress >= 92) return prev;
        return {
          ...prev,
          progress: Math.min(92, Number((prev.progress + 0.45).toFixed(2)))
        };
      });
    }, 1000);

    return () => window.clearInterval(timer);
  }, [skillInstallState.status]);
...
function PluginList({ channel, onOpenPay }: { channel: 'mock'|'wechat'|'alipay', onOpenPay: (p: { paymentId: string; title: string; amount: number; tokens?: number; pluginId?: string }) => void }) {
  const intl = useIntl();
  const [items, setItems] = useState<Array<{id:string;name:string;version:string;price:number;description:string;group?:string}>>([])
  const [loadingId, setLoadingId] = useState<string | null>(null)
  useEffect(() => {
    let alive = true
    const run = async () => {
      try {
        const locale = (intl.locale || '').trim()
        const r = await fetch(`/api/store/plugins?locale=${encodeURIComponent(locale)}`)
  ...
  const groupedItems = useMemo(() => {
    const groupMap = new Map<string, Array<(typeof items)[number]>>();
    const groupOrder = [
      '字体大小', 'Font Size',
      '图像工具', 'Image Tools',
      '表格工具', 'Table Tools',
    ...
    for (const item of items) {
      const group = (item.group || intl.formatMessage({ id: 'store.group.other', defaultMessage: '其他' })).trim() || intl.formatMessage({ id: 'store.group.other', defaultMessage: '其他' });
      if (!groupMap.has(group)) groupMap.set(group, []);
      groupMap.get(group)!.push(item);
    }

    const sections = Array.from(groupMap.entries()).map(([group, groupItems]) => ({
      group,
      items: groupItems.sort((a, b) => (b.price || 0) - (a.price || 0) || a.name.localeCompare(b.name)),
    }));

    sections.sort((a, b) => {
      const aIndex = groupOrder.findIndex((name) => name === a.group);
      const bIndex = groupOrder.findIndex((name) => name === b.group);
  ...
  return (
    <div className="space-y-5">
      {groupedItems.map((section) => (
        <section key={section.group} className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 shadow-[0_20px_60px_rgba(2,6,23,0.18)] backdrop-blur-xl md:p-5">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <div className="text-base font-bold text-white/95 md:text-lg">{section.group}</div>
              <div className="mt-1 text-xs text-slate-400">{section.items.length} 个功能</div>
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {section.items.map((it) => (
              <div key={it.id} data-enter-card className="flex min-h-[190px] flex-col rounded-lg border border-white/12 bg-slate-950/34 p-4 text-white shadow-[0_18px_50px_rgba(2,6,23,0.22)] backdrop-blur-xl">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="truncate text-base font-extrabold leading-tight md:text-lg">{it.name}</div>
                    <div className="mt-1 text-[11px] text-slate-300">v{it.version}</div>
                  </div>
                  <div className="shrink-0 rounded-md bg-white/8 px-2.5 py-1 text-sm font-black text-blue-300">
                    {it.price > 0 ? `¥${it.price}` : intl.formatMessage({ id: 'store.price.free', defaultMessage: '免费' })}
                  </div>
                </div>
                <div className="mt-3 line-clamp-3 min-h-[54px] flex-grow text-sm leading-6 text-slate-200">{it.description}</div>
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <button disabled={loadingId === it.id} onClick={() => purchase(it)} className="w-full rounded-md bg-gradient-to-r from-blue-600 to-purple-600 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
                  ...
                  <a
                    href={`/api/store/download?pluginId=${encodeURIComponent(it.id)}`}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center justify-center rounded-md border border-cyan-300/30 bg-cyan-400/10 py-2.5 text-sm font-semibold text-cyan-100 transition-colors hover:bg-cyan-400/18"
                  >
                    下载 jarv
                  </a>
                </div>
...
function PaymentModal({ info, channel, isInWordPlugin, onClose }: { info: { paymentId: string; title: string; amount: number; tokens?: number; pluginId?: string; modelType?: string; usageDurationLabel?: string; durationMonths?: number }; channel: 'mock'|'wechat'|'alipay'; isInWordPlugin: boolean; onClose: () => void }) {
  ...
  const [paymentConfirmMessage, setPaymentConfirmMessage] = useState<string>('')
  const pollRef = useRef<(() => Promise<boolean>) | null>(null)
  ...
  useEffect(() => {
    ...
    const gen = async () => {
      try {
        ...
        const isAlipayPagePay = channel === 'alipay' && /^https?:\/\//i.test(text)
        ...
        const d = await QRCode.toDataURL(text, { width: 256, errorCorrectionLevel: 'H', margin: 2 })
        const brandedQr = await addQrCenterLogo(d)
        if (alive) setQr(brandedQr)
      } catch (e) {
        if (alive) {
          setStatus('failed')
          setQr('')
    ...
    const poll = async (): Promise<boolean> => {
      ...
      try {
        if (channel === 'alipay' && paymentUrl) {
          try {
            const confirmRes = await fetch('/api/payment/confirm/alipay', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ orderNo: info.paymentId })
            })
            const confirmJson = await confirmRes.json()
            if (!confirmRes.ok && confirmJson?.error) {
              console.warn('Alipay confirmation failed:', confirmJson.error)
            }
          } catch (confirmError) {
            console.warn('Alipay confirmation request failed:', confirmError)
          }
        }
        ...
        if (sj?.success && sj?.status === 'paid') {
          ...
          const bearer = localStorage.getItem('token')
          const paidOrderType = String(sj?.orderType || '')
          const paidModelType = String(sj?.modelType || info.modelType || '')
          const isSubscriptionPayment = paidOrderType === 'subscription' || !!paidModelType
          const isTokenPackPayment = !isSubscriptionPayment && !!info.tokens && info.tokens > 0
    ...
    return () => { alive = false; pollRef.current = null; clearInterval(t) }
  }, [info.paymentId])
      ...
      <div className="bg-white rounded-2xl p-8 w-[420px]">
        ...
        {paymentUrl && (
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-center text-sm text-blue-700">
            {isZh ? '支付宝收银台已打开，请在新窗口完成支付。' : 'Alipay checkout opened in a new window.'}
          </div>
        )}
        <div className="text-center mt-4 text-sm text-gray-600">
          {paymentUrl
            ? (isZh ? '支付完成后请稍等，系统会自动确认订单。' : 'After payment, please wait while the order is confirmed.')
            : <FormattedMessage id="store.msg.scan_qr" defaultMessage="请使用支付工具扫描二维码或等待自动确认" />
          }
        </div>
        {paymentConfirmMessage && (
          <div className="mt-3 text-center text-xs text-amber-600">{paymentConfirmMessage}</div>
        )}
          ...
          {paymentUrl ? (
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const confirmed = await pollRef.current?.()
                  if (!confirmed) {
                    setPaymentConfirmMessage(isZh ? '还没有收到支付宝确认，请稍后再试。' : 'Payment is not confirmed yet. Please try again shortly.')
                  }
                } finally {
                  setIsConfirmingPayment(false)
                }
              }}
              className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg py-2 text-center disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isConfirmingPayment ? (isZh ? '正在确认…' : 'Checking...') : (isZh ? '我已付款' : 'I have paid')}
            </button>
          ) : (
        ...
        {status === 'success' && (info.modelType || !info.tokens) && (
        ...
        {status === 'success' && info.tokens && !info.modelType && (
          <div className="mt-4 text-center">
...
function SubscriptionPlans({ onOpenPay, channel }: { onOpenPay: (p: { paymentId: string; title: string; amount: number; tokens?: number; modelType?: string }) => void, channel: 'mock'|'wechat'|'alipay' }) {
  ...
  const fallbackModels = curatedDefaultModels.map(model => ({ id: model.id, name: model.name, value: model.id, provider: model.id }));
  ...
  const getProviderRank = (provider: string): number => {
    ...
    if (normalized === 'openai') normalized = 'gpt4';
    if (normalized === 'anthropic') normalized = 'claude';
    if (normalized === 'deepseek') return 0;
    if (normalized === 'kimi') return 1;
    if (normalized === 'qwen') return 2;
    if (normalized === 'doubao') return 3;
    if (normalized === 'zhipu') return 4;
    if (normalized === 'siliconflow') return 5;
    if (normalized === 'gpt4') return 6;
    if (normalized === 'claude') return 7;
  ...
  const visibleProviders = new Set(['deepseek', 'kimi', 'qwen', 'doubao', 'zhipu', 'siliconflow', 'gpt4', 'claude', 'gemini']);
  const displayModels = availableModels.length > 0 ? availableModels : fallbackModels;
  const visibleOrderedModels = displayModels
    .filter(model => visibleProviders.has(normalizeProviderForDisplay(model.provider)))
    .sort((a, b) => {
      const rankDiff = getProviderRank(a.provider) - getProviderRank(b.provider);
      if (rankDiff !== 0) return rankDiff;
