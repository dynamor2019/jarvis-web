...
// - normalized -> line 56
// - normalizePem -> line 63
// - resolveExistingPath -> line 68
// - POST -> line 99
// - wechatPayAppId -> line 251
// [/CodeGuard Feature Index]
    ...
    if (resolvedPaymentMethod === 'alipay') {
      ...
      if (config.alipay.appId && config.alipay.privateKey && (config.alipay.publicKey || hasCertMode)) {
        try {
          ...
          if (alipayPublicKey) {
            console.log('[AlipayRoute] sdk_mode=public_key');
            sdkOptions.privateKey = privateKey;
            sdkOptions.alipayPublicKey = alipayPublicKey;
          } else if (hasCertMode) {
            console.log('[AlipayRoute] sdk_mode=cert');
            sdkOptions.privateKey = privateKey;
          ...
          const precreateParams: Record<string, any> = {
            bizContent: {
          ...
          if (isPublicNotifyBaseUrl(baseUrl)) {
            precreateParams.notifyUrl = `${baseUrl}/api/payment/webhook/alipay`;
          }
          const precreateResult: any = await alipaySdk.exec(
            'alipay.trade.precreate',
            precreateParams
          );
          if (precreateResult?.code !== '10000' || !precreateResult?.qrCode) {
            const msg = precreateResult?.subMsg || precreateResult?.msg || 'Alipay precreate failed';
            throw new Error(msg);
          }
          qrText = precreateResult.qrCode;
