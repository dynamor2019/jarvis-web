...
function createMockConfigFile() {
  ...
  const deviceFingerprint = 'test_device_fingerprint_' + timestamp;
...
async function simulatePCConfigProcessing(configData) {
  ...
  try {
    ...
    // 模拟解密 - 提取API密钥
    const decodedData = Buffer.from(configData.encryptedData, 'base64').toString('utf-8');
    const parsedData = JSON.parse(decodedData);
    
    const apiKey = parsedData.apiKey;
    const modelType = parsedData.modelType;
