...
print_recent_logs() {
    echo "---------------- logs/out.log (tail -n 80) ----------------"
    tail -n 80 ./logs/out.log 2>/dev/null || true
    echo "---------------- logs/err.log (tail -n 80) ----------------"
...
wait_for_health() {
    ...
    for i in $(seq 1 "$attempts"); do
        if ! kill -0 "$WEB_PID" >/dev/null 2>&1; then
            echo "CRITICAL: Node process exited while waiting for health check."
            return 1
        fi
...
echo "Starting Node server directly..."
nohup node "$SERVER_FILE" > ./logs/out.log 2> ./logs/err.log &
WEB_PID=$!
echo "$WEB_PID" > ./jarvis-web.pid

sleep 3

if ! kill -0 "$WEB_PID" >/dev/null 2>&1; then
    echo "CRITICAL: Node process exited during startup."
    print_recent_logs
    exit 1
fi
