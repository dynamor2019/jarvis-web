...
const techStack = [
    ['React', 'tech.stack.0.role', 'tech.stack.0.desc'],
    ['TypeScript', 'tech.stack.1.role', 'tech.stack.1.desc'],
    ['Node.js', 'tech.stack.2.role', 'tech.stack.2.desc'],
    ['C# VSTO', 'tech.stack.3.role', 'tech.stack.3.desc'],
    ['Security', 'tech.stack.4.role', 'tech.stack.4.desc'],
    ['Crypto', 'tech.stack.5.role', 'tech.stack.5.desc'],
];

const architectureLayers = [
    {
        name: 'Web',
        titleId: 'tech.layer.0.title',
        descId: 'tech.layer.0.desc',
        color: '#60a5fa'
    },
    {
        name: 'Service',
        titleId: 'tech.layer.1.title',
        descId: 'tech.layer.1.desc',
        color: '#34d399'
    },
    {
        name: 'PC',
        titleId: 'tech.layer.2.title',
        descId: 'tech.layer.2.desc',
        color: '#fbbf24'
    }
];

const securityTags = ['tech.security.0', 'tech.security.1', 'tech.security.2'];
const localTags = ['tech.local.0', 'tech.local.1', 'tech.local.2', 'tech.local.3'];
            ...
            <div className="container relative mx-auto px-4">
                <div className="mb-4">
                    <div className="mx-auto max-w-3xl text-center">
                        <div className="mb-2 inline-flex rounded-full border border-emerald-200/20 bg-white/8 px-4 py-1 text-sm font-semibold text-emerald-100 shadow-sm backdrop-blur-xl">
                            <FormattedMessage id="tech.badge" defaultMessage="工程底座" />
                        </div>
                    ...
                    <div className="mx-auto mt-4 grid max-w-3xl gap-3 sm:grid-cols-3">
                        {['tech.tag.0', 'tech.tag.1', 'tech.tag.2'].map((id) => (
                            <div key={id} className="rounded-2xl border border-white/10 bg-white/[0.055] px-4 py-2.5 text-center text-sm font-semibold text-emerald-50 backdrop-blur-xl">
                    ...
                    <div className="relative">
                        <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
                            <div>
                                <div className="text-base font-semibold text-emerald-100">
                                    <FormattedMessage id="tech.panel.title" defaultMessage="技术蓝图" />
                                </div>
                                <div className="mt-1 text-sm leading-5 text-slate-300">
                                    <FormattedMessage id="tech.panel.subtitle" defaultMessage="三层架构独立协作，安全协议负责连接和保护。" />
                                </div>
                            </div>
                            <div className="rounded-full border border-emerald-200/20 bg-emerald-300/10 px-3 py-1 text-xs font-semibold text-emerald-100">
                                <FormattedMessage id="tech.panel.badge" defaultMessage="架构视图" />
                            </div>
                        </div>
                            ...
                            {architectureLayers.map((layer, index) => (
                                <div key={layer.name} className="contents">
                                    <div className="relative min-h-[170px] rounded-[22px] border border-white/10 bg-white/[0.06] p-5">
                                        ...
                                        <div className="text-2xl font-black leading-tight text-white"><FormattedMessage id={layer.titleId} defaultMessage={layer.titleId} /></div>
                                        <div className="mt-2 text-sm font-medium leading-5 text-slate-200"><FormattedMessage id={layer.descId} defaultMessage={layer.descId} /></div>
                                            ...
                                            {techStack.slice(index * 2, index * 2 + 2).map((item) => (
                                                <div key={item[0]} className="rounded-xl border border-white/10 bg-slate-950/38 px-3 py-2">
                                                    <div className="flex items-center justify-between gap-2">
                                                        ...
                                                        <div className="text-base font-bold text-white"><FormattedMessage id={item[1]} defaultMessage={item[1]} /></div>
                                                    </div>
                                                    <div className="mt-1 text-xs leading-5 text-slate-300"><FormattedMessage id={item[2]} defaultMessage={item[2]} /></div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                        ...
                        <div className="mt-3 grid gap-3 lg:grid-cols-[1.2fr_0.8fr]">
                            <div className="rounded-[20px] border border-emerald-200/12 bg-emerald-300/8 p-3">
                                <div className="mb-2 text-xs font-semibold text-emerald-100">
                                    <FormattedMessage id="tech.security.title" defaultMessage="安全协议与配置保护" />
                                </div>
                                <div className="grid gap-2 text-sm font-semibold text-slate-200 sm:grid-cols-3">
                                    {securityTags.map((id) => (
                                        <div key={id} className="rounded-xl border border-white/10 bg-slate-950/34 px-3 py-2 text-center">
                                            <FormattedMessage id={id} defaultMessage={id.split('.').pop() || ''} />
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="rounded-[20px] border border-white/10 bg-white/[0.055] p-3">
                                <div className="mb-2 text-xs font-semibold text-emerald-100">
                                    <FormattedMessage id="tech.local.title" defaultMessage="本地可信执行" />
                                </div>
                                <div className="grid grid-cols-2 gap-2 text-sm font-semibold text-slate-200">
                                    {localTags.map((id) => (
                                        <div key={id} className="rounded-xl bg-slate-950/34 px-3 py-2 text-center">
                                            <FormattedMessage id={id} defaultMessage={id.split('.').pop() || ''} />
                                        </div>
                                    ))}
                                </div>
