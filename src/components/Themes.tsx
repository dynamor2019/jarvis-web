...
export default function Themes() {
    ...
    const activeSkin = skinThemes[activeSkinIndex];
    ...
    useEffect(() => {
        ...
        let step = 0;
        const maxSteps = skinThemes.length * 2 - 1;
        const timer = window.setInterval(() => {
            ...
            step += 1;
            setActiveSkinIndex(step % skinThemes.length);
        }, 3200);

        return () => window.clearInterval(timer);
    }, [isSectionVisible, replayKey]);

    return (
        <section ref={sectionRef} id="themes" className="relative flex min-h-[calc(100vh-64px)] items-center overflow-hidden bg-[#050816] py-6 text-white md:py-7" style={{ scrollMarginTop: '80px' }}>
            <style jsx>{`
                @keyframes theme-section-scale-in {
                    0% {
                        opacity: 0.42;
                ...
                @keyframes skin-door-in {
                    0% {
                        opacity: 0;
                        transform: perspective(1200px) rotateY(-76deg) translateX(62px) scale(0.9);
                        filter: saturate(0.9) brightness(0.9);
                    }
                    46% {
                        opacity: 1;
                        transform: perspective(1200px) rotateY(16deg) translateX(0) scale(1.015);
                        filter: saturate(1.14) brightness(1.04);
                    }
                    72% {
                        transform: perspective(1200px) rotateY(-4deg) translateX(0) scale(1);
                    }
                    100% {
                        opacity: 1;
                ...
                .theme-section-enter {
                    animation: theme-section-scale-in 860ms cubic-bezier(0.22, 0.74, 0.24, 1) both;
            ...
            <div className="pointer-events-none absolute inset-0">
                <img src="/assets/theme-workspace-bg.jpg" alt="" className="h-full w-full object-cover opacity-34 mix-blend-luminosity" />
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_14%_16%,rgba(34,211,238,0.24),transparent_31%),radial-gradient(circle_at_84%_24%,rgba(236,72,153,0.22),transparent_30%),radial-gradient(circle_at_50%_92%,rgba(79,70,229,0.16),transparent_38%),linear-gradient(180deg,rgba(5,8,22,0.78)_0%,rgba(15,23,42,0.82)_58%,rgba(17,24,39,0.88)_100%)]" />
                <div className="absolute left-10 right-10 top-24 hidden h-[380px] rounded-[44px] border border-cyan-200/[0.045] bg-[linear-gradient(115deg,rgba(34,211,238,0.045),rgba(236,72,153,0.035),rgba(255,255,255,0.015))] opacity-70 shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_34px_100px_rgba(2,6,23,0.22)] backdrop-blur-[2px] lg:block" />
                <div className="absolute left-16 top-28 hidden h-px w-56 bg-gradient-to-r from-transparent via-cyan-100/20 to-transparent lg:block" />
            </div>
            <div key={replayKey} className="theme-section-enter container relative mx-auto px-4">
                <div className="mx-auto mb-5 max-w-4xl text-center">
                    <h2 className="mb-3 text-4xl font-black leading-tight text-white md:text-5xl">
                        <FormattedMessage id="themes.heading.prefix" />
                        <span className="gradient-text"><FormattedMessage id="themes.heading.emphasis" /></span>
                    </h2>
                    <p className="mx-auto max-w-3xl text-sm leading-6 text-slate-300 md:text-base">
                ...
                <div className="grid items-stretch gap-5 lg:grid-cols-[0.88fr_1.12fr]">
                    <div className="flex h-full flex-col rounded-[30px] border border-white/12 bg-white/[0.06] p-5 shadow-[0_30px_100px_rgba(2,6,23,0.4)] backdrop-blur-2xl">
                        ...
                        <div className="relative min-h-[410px] flex-1 overflow-hidden rounded-[26px] border border-white/10 bg-slate-950/32">
                            ...
                            <div className={`absolute inset-x-6 bottom-6 top-[78px] rounded-[24px] border border-white/10 bg-gradient-to-br ${activeSkin.accent} p-[1px] shadow-[0_26px_80px_rgba(2,6,23,0.42)] [perspective:1400px]`}>
                                <div className="relative flex h-full items-center justify-center overflow-hidden rounded-[23px] bg-white/5 backdrop-blur-sm transition-colors duration-1000">
                                    <div className={`absolute inset-0 bg-gradient-to-br ${activeSkin.surface} opacity-40 transition-opacity duration-1000`} />
                                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_26%_20%,rgba(255,255,255,0.46),transparent_34%),linear-gradient(180deg,rgba(255,255,255,0.12),rgba(255,255,255,0.02))] transition-opacity duration-1000" />
                                    <div
                                        key={activeSkin.image}
                                        className="relative flex h-full w-full items-center justify-center [transform-origin:left_center] [transform-style:preserve-3d]"
                                        style={{ animation: 'skin-door-in 2200ms cubic-bezier(0.2, 0.72, 0.18, 1) both' }}
                                    >
                                        <img
                                            src={activeSkin.image}
                                            alt=""
                                            className="h-full w-full object-cover object-center brightness-110 saturate-125 drop-shadow-[0_24px_60px_rgba(0,0,0,0.34)]"
                                        />
                                    </div>
                                </div>
                            </div>
                            <div className="pointer-events-none absolute left-1/2 top-1/2 h-44 w-44 -translate-x-1/2 -translate-y-1/2 rounded-full border border-cyan-200/10 bg-cyan-200/5 blur-xl" />
                        </div>
                    </div>

                    <div className="relative overflow-hidden rounded-[30px] border border-white/14 bg-slate-950/44 p-5 shadow-[0_30px_100px_rgba(2,6,23,0.4)] backdrop-blur-2xl">
                        <div className="absolute -right-16 -top-20 h-64 w-64 rounded-full bg-violet-300/14 blur-3xl" />
                        <div className="absolute -bottom-20 left-10 h-64 w-64 rounded-full bg-cyan-300/12 blur-3xl" />
                        <div className="relative flex h-full flex-col">
                            ...
                            <div className="grid flex-1 gap-4 xl:grid-cols-[1fr_0.74fr]">
                                <div className="relative min-h-[370px] overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(135deg,rgba(15,23,42,0.96),rgba(67,56,202,0.22),rgba(8,47,73,0.78))]">
                                    ...
                                    <div className="absolute inset-x-8 top-1/2 -translate-y-1/2 rounded-[30px] border border-white/12 bg-white/[0.055] px-6 py-10 shadow-[0_28px_80px_rgba(2,6,23,0.42)] backdrop-blur-xl">
                                        <img
                                            ...
                                            alt=""
                                            className="mx-auto w-full max-w-[524px] rounded-[24px] object-contain"
                                        />
                                    </div>
